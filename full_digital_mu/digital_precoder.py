...
def qr_with_positive_diagonal(channel_block: np.ndarray) -> np.ndarray:
    """Return the reduced QR upper factor with positive diagonal phases."""
...
def build_fd_svd_precoder(channel: np.ndarray, num_streams: int) -> np.ndarray:
    """Build a fully-digital SVD precoder from the channel right singular vectors."""
...
class DigitalStructuredPrecoder:
    """
    Structured digital precoder on the per-user reduced channel.

    This module is shared by the fully-digital workflow, where the effective
    channel is simply the physical user channel after BD projection.
    """
    ...
    def _design_strict_ucd(
        ...
        omega, _ = self._apply_strict_gmd(

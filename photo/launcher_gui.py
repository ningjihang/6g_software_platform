...
TRAIN_TASK_TO_SCRIPT = {
    "Hybrid MU Precoding": HYBRID_TRAIN_SCRIPT,
    "Digital Precoding": DIGITAL_TRAIN_SCRIPT,
}
TRAIN_TASK_TO_RESULT_DIR = {
    "Hybrid MU Precoding": ROOT_DIR / "classical" / "results",
    "Digital Precoding": ROOT_DIR / "full_digital_mu" / "results",
}

ALL_OPTION = "ALL"
TRAIN_QAM_OPTIONS = ("QPSK", "4QAM", "16QAM", "64QAM", "256QAM")
TRAIN_CSI_MODE_OPTIONS = ("有误差", "无误差")
...
FULL_DIGITAL_IMAGE_PATTERN = re.compile(
    r"^(?P<label>(?:compare_full_digital_(?:svd_vs_gmd_thp|svd_gmd_thp_ucd|svd_gmd_ucd)|全数字_SVD_GMD(?:THP)?_UCD对比))_(?P<channel>[A-Za-z0-9-]+)_k(?P<users>\d+)_nt(?P<tx>\d+)_nr(?P<rx>\d+)(?:_ns(?P<ns>\d+))?.*?_m(?P<bits>\d+)_.*\.png$",
    re.IGNORECASE,
)
...
class PhotoEntry:
    ...
    def title(self) -> str:
        return (
            ...
            f"{self.channel_model} | {self.channel_estimation} | Nt={self.tx} Nr={self.rx} | "
            f"{self.qam} | rate {self.rate}"
        )
...
class LauncherGuiApp:
    def __init__(self, root: tk.Tk) -> None:
        ...
        self.m2_status_var = tk.StringVar(value="Not started")
    ...
    def _normalize_channel_estimation(value: object) -> str:
        ...
        if compact in {"perfect", "perfectcsi", "noerror"}:
            return "无误差"
        if compact in {"na", "none", ""}:
            return "N/A"
        return "有误差"
    ...
    @classmethod
    def _rate_sort_key(cls, value: str) -> tuple[int, float, str]:
        text = cls._normalize_rate(value)
        frac_match = re.match(r"^(\d+)\s*/\s*(\d+)$", text)
        if frac_match is not None:
            num = int(frac_match.group(1))
            den = int(frac_match.group(2))
            return (0, num / den if den else float("inf"), text)
        try:
        ...
        for image_path in sorted(FULL_DIGITAL_RESULTS_DIR.glob("*.png")):
            ...
            entries.append(
                PhotoEntry(
                    ...
                    qam=self._qam_from_bits(match.group("bits")),
                    rate="N/A",
                    file=image_path.name,
                    path=image_path,
                )
    ...
    def _build_layout(self) -> None:
        ...
        ttk.Label(outer, text="Match Photo / Train Modes", style="Title.TLabel").pack(anchor="w")
        ttk.Label(
            outer,
            text="Match Photo uses existing image configs only; Train Mode supports custom settings.",
            style="Hint.TLabel",
    ...
    def _build_mode1_panel(self, parent: ttk.Frame) -> ttk.Frame:
        ...
        ttk.Label(card, text="Match Photo: Single-User / Multi-User", style="CardTitle.TLabel").grid(
        ...
        self.m1_user_scope_combo = ttk.Combobox(
            card,
            textvariable=self.m1_user_scope_var,
            state="readonly",
            width=14,
        )
        ...
        ttk.Label(card, text="信道误差", style="CardText.TLabel").grid(row=2, column=8, sticky="w", padx=(12, 0), pady=(10, 0))
        self.m1_csi_combo = ttk.Combobox(card, textvariable=self.m1_csi_var, state="readonly", width=12)
        ...
        button_row.grid(row=3, column=0, columnspan=10, sticky="ew", pady=(14, 0))
        ...
        ttk.Label(
            ...
            justify="left",
            wraplength=900,
        ).grid(row=4, column=0, columnspan=10, sticky="w", pady=(12, 0))
        ...
        self.m1_image_container.grid(row=5, column=0, columnspan=10, sticky="nsew", pady=(12, 0))
    ...
    def _build_mode2_panel(self, parent: ttk.Frame) -> ttk.Frame:
        ...
        ttk.Label(control_card, text="Train Mode: Custom Setup", style="CardTitle.TLabel").grid(
            row=0, column=0, columnspan=4, sticky="w"
        )

        ttk.Label(control_card, text="Training Task", style="CardText.TLabel").grid(row=1, column=0, sticky="w", pady=(10, 0))
        ...
        ttk.Label(control_card, text="Channel Library", style="CardText.TLabel").grid(row=1, column=2, sticky="w", pady=(10, 0))
        ...
        ttk.Label(control_card, text="Input Antennas Nt", style="CardText.TLabel").grid(row=2, column=0, sticky="w", pady=(10, 0))
        ...
        ttk.Label(control_card, text="Output Antennas Nr", style="CardText.TLabel").grid(row=2, column=2, sticky="w", pady=(10, 0))
        ...
        ttk.Label(control_card, text="QAM", style="CardText.TLabel").grid(row=4, column=2, sticky="w", pady=(10, 0))
        ...
        self.m2_qam_combo.grid(row=4, column=3, sticky="w", padx=(8, 0), pady=(10, 0))
        ...
        self.m2_csi_label.grid(row=5, column=0, sticky="w", pady=(10, 0))
        self.m2_csi_combo.grid(row=5, column=1, sticky="w", padx=(8, 12), pady=(10, 0))

        button_row = ttk.Frame(control_card, style="Card.TFrame")
        button_row.grid(row=6, column=0, columnspan=4, sticky="ew", pady=(14, 0))
        button_row.columnconfigure(0, weight=1)
        button_row.columnconfigure(1, weight=1)
        button_row.columnconfigure(2, weight=1)

        self.start_train_button = ttk.Button(button_row, text="Start Train", command=self._start_training)
        ...
        ttk.Label(log_card, text="Training Log", style="CardTitle.TLabel").grid(row=0, column=0, sticky="w")
    ...
    def _mode1_filter_order() -> tuple[str, ...]:
        return ("user_scope", "precoding_family", "metric", "channel", "csi", "tx", "rx", "qam", "rate")
        ...
        if field == "user_scope":
            ...
            if field == "precoding_family":
                ...
                if field == "metric":
                    ...
                    values = sorted({item.rate for item in entries}, key=self._rate_sort_key)
                else:
                    values = []
    ...
    def _update_mode1_status(self) -> None:
        ...
        self.m1_status_var.set(
            ...
            f"{first.channel_estimation} | {first.file}"
        )
    ...
    def _match_mode1_photo(self) -> None:
        ...
        self.m1_status_var.set(
            ...
            f"{selected.channel_model} | {selected.channel_estimation} | {selected.file}"
        )
    ...
    def _start_training(self) -> None:
        ...
        if script_path is None or not script_path.exists():
            messagebox.showerror("Script Missing", f"Training script not found:\n{script_path}")
            return
        ...
        try:
            ...
            messagebox.showerror("Invalid Parameters", "Nt and Nr must be positive integers.")
            return
        ...
        if bits_per_symbol is None:
            messagebox.showerror("Invalid Parameters", "QAM must be one of QPSK, 4QAM, 16QAM, 64QAM, or 256QAM.")
            return
        ...
        if is_hybrid:
            ...
            if topology_error is not None:
                messagebox.showerror("Invalid Parameters", topology_error)
                return
            ...
            extra_summary_parts.extend(
                [
                    ...
                    f"Ns={num_streams}",
                    f"Nrf={num_rf_chains}",
                    f"CSI={self.m2_csi_var.get().strip()}",
        ...
        self._append_log(f"\n>>> Start Train: {' '.join(command)}")
        self.m2_status_var.set(
            f"Training: {task} | channel={channel_type} | Nt={tx} Nr={rx} | "
    ...
    def _on_training_finished(self, return_code: int) -> None:
        self._append_log(f">>> Training finished, return code: {return_code}")
        if return_code == 0:
            self.m2_status_var.set("Training completed")
        else:
            self.m2_status_var.set(f"Training failed (code {return_code})")
    ...
    def _stop_training(self) -> None:
        ...
        if process is None or process.poll() is not None:
            self.m2_status_var.set("No running training process")
            self.stop_train_button.configure(state="disabled")
            self.start_train_button.configure(state="normal")
            return

        self._append_log(">>> Request stop training ...")
        process.terminate()
        self.m2_status_var.set("Stopping training ...")
    ...
    def _force_kill_training_if_needed(self) -> None:
        ...
        if process is None or process.poll() is not None:
            return
        process.kill()
        self._append_log(">>> Training process force-killed.")

    def _open_selected_result_dir(self) -> None:
        task = self.m2_task_var.get().strip()
        result_dir = TRAIN_TASK_TO_RESULT_DIR.get(task)
        if result_dir is None:
            messagebox.showerror("Path Error", f"Unknown training task: {task}")
            return
        ...
        if not self._open_with_system_viewer(result_dir):
            ...
            try:
                ...
                pass
            messagebox.showerror("Open Failed", f"Cannot open folder:\n{result_dir}")
